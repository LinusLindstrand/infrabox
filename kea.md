...
Each server is hosted as an LXC and both servers listen on interface `eth0` and receive DHCP requests **relayed by OPNsense** if not on the same broadcast domain. OPNsense forwards client DISCOVER/REQUEST packets as unicast to both Kea servers.
